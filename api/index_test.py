"""
Modified index.py for testing without external dependencies.
This file uses mock implementations instead of real ones.
"""
...
mocks_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "mocks")
sys.path.insert(0, mocks_path)
logger.info(f"Added mocks path to sys.path: {mocks_path}")
...
async def lifespan(app: FastAPI):
    ...
    logger.info("API starting up in test mode")
    yield
...
try:
    ...
    logger.info("Successfully imported mock modules")
except ImportError as e:
    logger.error(f"Error importing mock modules: {e}")
    raise
...
async def get_connection():
    ...
    if _connection is None or not _connection.isconnected():
        try:
            ...
            logger.info(f"Connected to mock SAP HANA Cloud at {DB_HOST}:{DB_PORT}")
        except Exception as e:
            logger.error(f"Failed to connect to SAP HANA Cloud: {str(e)}")
            raise RuntimeError(f"Database connection failed: {str(e)}")
    return _connection

async def vector_search(query: str, k: int = 4, filter: Optional[Dict] = None, 
                      use_mmr: bool = False, lambda_mult: float = 0.5, fetch_k: int = 20) -> List[DocumentWithScore]:
    """
    Mock vector search function that simulates SAP HANA Cloud search.
    """
    logger.info(f"Performing mock vector search for query: '{query}', k={k}, use_mmr={use_mmr}")
...
async def root():
    """Root endpoint."""
    return {
        "message": "SAP HANA Cloud LangChain Integration API (Test Mode)",
        "version": "1.2.0",
        "features": [
...
async def health_status():
    """Health status endpoint with detailed status."""
    return {
        "status": "ok",
        "components": [
            {
...
async def vector_search_api(search_query: SearchQuery):
    """
    Perform vector similarity search against SAP HANA Cloud.
    
    This endpoint simulates vector similarity search using mock implementations.
    """
    start_time = time.time()
    
    try:
        # Use mock vector search implementation
