...
# Try to import GPU-specific embeddings, but don't fail if unavailable
try:
    # These imports will only succeed in environments with GPU support
    try:
        from gpu_embeddings import TensorRTEmbeddings, GPUAcceleratedEmbeddings
        GPU_EMBEDDING_AVAILABLE = True
    except ImportError:
        # GPU embeddings not available - will use CPU fallbacks
        GPU_EMBEDDING_AVAILABLE = False
except ImportError:
    # If any import fails, we'll still have HanaInternalEmbeddings
    pass
...
import gpu_utils
from models import (
    Flow, DebugSession, DebugNodeData,
...
def generate_code_from_flow(flow: Flow) -> str:
    ...
    connection_nodes = [node for node in flow.nodes if node.data.type == 'connection']
    vectorstore_nodes = [node for node in flow.nodes if node.data.type == 'vectorstore']
    embedding_nodes = [node for node in flow.nodes if node.data.type == 'embedding']
    llm_nodes = [node for node in flow.nodes if node.data.type == 'llm']
    memory_nodes = [node for node in flow.nodes if node.data.type == 'memory']
    prompt_nodes = [node for node in flow.nodes if node.data.type == 'prompt']
    chain_nodes = [node for node in flow.nodes if node.data.type == 'chain']
...
from langchain_hana import HanaDB  # Was HanaVectorStore
from langchain_hana.embeddings import HanaInternalEmbeddings

# Connect to SAP HANA Cloud
import hdbcli.dbapi

def connection_code(connection_nodes):
    """Generate connection code for HANA database.
    
    Args:
        connection_nodes: List of connection nodes with parameters
        
    Returns:
        str: Generated Python code for database connection
    """
    if not connection_nodes:
        return "# No connection parameters provided"
    ...
    return f"""# HANA Database Connection
...
"""
    ...
    if embedding_nodes:
        ...
        if embedding.get('useTensorRT', False):
            code += f'''
# Try TensorRT-optimized embeddings first, with fallbacks
try:
    # Attempt to use TensorRT embeddings
    try:
        # Try TensorRT embeddings if GPU is available
        ...
        print("Using TensorRT-optimized embeddings")
    except (ImportError, Exception) as e:
        print(f"Error initializing TensorRT embeddings: {{str(e)}}")
        print("Falling back to CPU embeddings")
        # Fall back to CPU embeddings
        raise ImportError("TensorRT unavailable")
except (ImportError, Exception) as e:
    # Fall back to HanaInternalEmbeddings
    embeddings = HanaInternalEmbeddings(
        model_name="{model_name}"
    )
    print("Using CPU-compatible HanaInternalEmbeddings")
    print("(fallback from TensorRT)")
'''
        elif embedding.get('useGPU', False):
            code += f'''
# Try GPU-accelerated embeddings first, with fallbacks
try:
    # Attempt to use GPU-accelerated embeddings
    import gpu_utils
...
"""
            ...
            print(f"GPUAcceleratedEmbeddings unavailable: {{e}}")
            raise
    else:
        raise ImportError("GPU not available")
except (ImportError, Exception) as e:
    print(f"GPU embeddings unavailable: {{e}}")
    print("Falling back to CPU embeddings.")
    
    # Fall back to CPU embeddings
    code += f"""
# Initialize CPU-compatible embeddings
embeddings = HanaInternalEmbeddings(
    model_name="{model_name}"
)
print("Using CPU-compatible HanaInternalEmbeddings")
print("(fallback from GPU)")
"""
        else:
            # Default to CPU embeddings
            code += f"""
# Initialize CPU-compatible embeddings
embeddings = HanaInternalEmbeddings(
    model_name="{model_name}"
...
def run_flow(flow: Flow) -> RunFlowResponse:
    ...
    try:
        ...
        if embedding_nodes:
            ...
            # Initialize with robust fallbacks between GPU and CPU options
            embeddings = None
            
            # Try TensorRT embeddings if requested and GPU available
            ...
            if embeddings is None and use_gpu and gpu_utils.is_gpu_available():
                try:
                    embeddings = GPUAcceleratedEmbeddings(
                        model_name=model_name,
                        device="cuda",
